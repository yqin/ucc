...
#ifndef ALLGATHERV_OFFLOAD_DPU_H_
#define ALLGATHERV_OFFLOAD_DPU_H_
...
static size_t
unpack_allgatherv_offload_args(void *data, allgatherv_offload_args_t *args)
{
    int i;
    size_t total_size = 0;
...
static int create_allgatherv_offload_subops(allgatherv_offload_coll_t *op)
{
    ...
    for (i = 1; i < size; i++) {
        /* actual peer rank */
        peer = (rank + i) % size;
        allgatherv_offload_subop_t *subop =
            malloc(sizeof(allgatherv_offload_subop_t));
        if (!subop) {
            ucs_error("not enough memory");
            return UCS_ERR_NO_MEMORY;
        }
        ...
        subop->type   = ALLGATHERV_OFFLOAD_SEND;
        subop->peer   = peer;
    ...
    for (i = 1; i < size; i++) {
        ...
        op->r_todo++;
        subop->status = UCS_INPROGRESS;
        subop->type   = ALLGATHERV_OFFLOAD_RECV;
        subop->peer   = peer;
...
/* calculate the size of allgatherv_offload_dpu_rts_t data structure */
static size_t get_allgatherv_offload_dpu_rts_packed_size(size_t rkey_len)
{
    size_t total_size = 0;
    ...
    total_size += FIELD_SIZEOF(allgatherv_offload_dpu_rts_t, start);
    total_size += FIELD_SIZEOF(allgatherv_offload_dpu_rts_t, length);
    total_size += FIELD_SIZEOF(allgatherv_offload_dpu_rts_t, rkey_len);
    total_size += rkey_len;
...
static size_t
pack_allgatherv_offload_dpu_rts(allgatherv_offload_coll_t *op,
...
static int
add_receive(execution_context_t *context, allgatherv_offload_coll_t *op,
            allgatherv_offload_subop_t *subop,
            allgatherv_offload_dpu_rts_t *args)
{
    int rc;
    ...
    allgatherv_offload_rdma_read_t *item =
        malloc(sizeof(allgatherv_offload_rdma_read_t));
    if (!item) {
        ucs_error("not enough memory");
        return UCS_ERR_NO_MEMORY;
    }
    ...
    memcpy(item->s_rkey_buf, (void *)args +
           FIELD_OFFSET(allgatherv_offload_dpu_rts_t, rkey_buf),
           args->rkey_len);
...
static int
complete_op(execution_context_t *context, allgatherv_offload_coll_t *op)
{
    int rc;
    ...
    size_t packed_size = sizeof(allgatherv_offload_dpu_done_t);
                  ...
                  size_t hdr_size, void *data, size_t data_len)
{
    ...
    allgatherv_offload_coll_t *op = malloc(sizeof(allgatherv_offload_coll_t));
    if (!op) {
        ucs_error("not enough memory");
        return UCS_ERR_NO_MEMORY;
    }
    ...
    allgatherv_offload_args_t *args = &op->args;
    size_t unpacked_size = unpack_allgatherv_offload_args(data, args);
    ...
    /* prepare subops */
    rc = create_allgatherv_offload_subops(op);
    if (rc) {
        ucs_error("create_allgatherv_offload_subops() failed");
        return rc;
    }
    ...
    ucs_list_for_each_safe(subop_item, subop_tmp, &op->s_pending, super) {
        ...
        /* get an event from event pool, allocate event buffer */
        size_t packed_size = get_allgatherv_offload_dpu_rts_packed_size(
                                op->s_rkey_len);
        dpu_offload_event_info_t event_info = { .payload_size = packed_size, };
        dpu_offload_event_t *event;
        ...
        size_t packed_size_tmp = pack_allgatherv_offload_dpu_rts(op,
                                    subop->peer, event->payload);
        assert(packed_size == packed_size_tmp);

        /* send RTS notification to remote DPU about data readiness */
        rc = event_channel_emit(&event, UCC_TL_UCP_ALLGATHERV_DPU_RTS_AM_ID,
              ...
              size_t data_len)
{
    ...
    allgatherv_offload_dpu_rts_t *args = (allgatherv_offload_dpu_rts_t *)data;
    int rc;
    ...
    if (op == NULL) {
        ...
        allgatherv_offload_dpu_rts_event_t *item =
            malloc(sizeof(allgatherv_offload_dpu_rts_event_t));
        if (!item) {
            ucs_error("not enough memory");
            return UCS_ERR_NO_MEMORY;
        }
        ...
        allgatherv_offload_dpu_rts_t *payload = malloc(data_len);
              ...
              size_t data_len)
{
    ...
    allgatherv_offload_dpu_ack_t *args = (allgatherv_offload_dpu_ack_t *)data;
    int rc;
...
static int register_allgatherv_dpu_notifications(offloading_engine_t *engine)
{
    int rc;
...
static int progress_rts(execution_context_t *context)
{
    ...
    allgatherv_offload_dpu_rts_event_t *rts = NULL, *rts_item, *rts_tmp;
    allgatherv_offload_coll_t *op = NULL, *op_item, *op_tmp;
    allgatherv_offload_subop_t *subop = NULL, *subop_item, *subop_tmp;
...
static int progress_receive(execution_context_t *context)
{
    ...
    allgatherv_offload_rdma_read_t *recv = NULL, *recv_item, *recv_tmp;
    allgatherv_offload_coll_t *op;
    allgatherv_offload_subop_t *subop;
    ucs_list_for_each_safe(recv_item, recv_tmp, &posted_receives, super) {
        ...
        if (status == UCS_OK) {
            ...
            /* get an event from event pool, allocate event buffer */
            size_t packed_size = sizeof(allgatherv_offload_dpu_ack_t);
    ...
    for (int i = 0; (i < max_receives - posted_count) &&
        ...
        allgatherv_offload_rdma_read_t *item =
            ucs_list_extract_head(&pending_receives,
                                  allgatherv_offload_rdma_read_t, super);
...
#endif //ALLGATHERV_OFFLOAD_DPU_H_
