...
#ifndef IBCAST_OFFLOAD_DPU_H_
#define IBCAST_OFFLOAD_DPU_H_
...
static size_t
unpack_ibcast_offload_args(void *data, ibcast_offload_args_t *args)
{
    int i;
    size_t total_size = 0;
...
static int create_ibcast_offload_subops(ibcast_offload_coll_t *op)
{
    ...
    for (i = 1; i < size; i++) {
        /* actual peer rank */
        peer = (rank + i) % size;
        ibcast_offload_subop_t *subop =
            malloc(sizeof(ibcast_offload_subop_t));
        if (!subop) {
            ucs_error("not enough memory");
            return UCS_ERR_NO_MEMORY;
        }
        ...
        subop->type   = IBCAST_OFFLOAD_SEND;
        subop->peer   = peer;
    ...
    for (i = 1; i < size; i++) {
        ...
        op->r_todo++;
        subop->status = UCS_INPROGRESS;
        subop->type   = IBCAST_OFFLOAD_RECV;
        subop->peer   = peer;
...
/* calculate the size of ibcast_offload_dpu_rts_t data structure */
static size_t get_ibcast_offload_dpu_rts_packed_size(size_t rkey_len)
{
    size_t total_size = 0;

    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, coll_type);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, tag);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, group_id);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, rank);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, peer);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, padding);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, start);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, length);
    total_size += FIELD_SIZEOF(ibcast_offload_dpu_rts_t, rkey_len);
    total_size += rkey_len;
...
static size_t
pack_ibcast_offload_dpu_rts(ibcast_offload_coll_t *op,
...
static int
add_receive(execution_context_t *context, ibcast_offload_coll_t *op,
            ibcast_offload_subop_t *subop,
            ibcast_offload_dpu_rts_t *args)
{
    int rc;
    ...
    ibcast_offload_rdma_read_t *item =
        malloc(sizeof(ibcast_offload_rdma_read_t));
    if (!item) {
        ucs_error("not enough memory");
        return UCS_ERR_NO_MEMORY;
    }
    ...
    item->r_start = op->args.r_start + op->args.r_displacements[args->rank];    /* TODO: */
    ...
    memcpy(item->s_rkey_buf, (void *)args +
           FIELD_OFFSET(ibcast_offload_dpu_rts_t, rkey_buf),
           args->rkey_len);
...
static int
complete_op(execution_context_t *context, ibcast_offload_coll_t *op)
{
    int rc;
    ...
    size_t packed_size = sizeof(ibcast_offload_dpu_done_t);
    ...
    ibcast_offload_dpu_done_t *args_buf = event->payload;
    ...
    dest_client_t c = GET_CLIENT_BY_RANK(host_context, op->args.group_id,
                                         op->args.rank);
    assert(c.ep);
    rc = event_channel_emit(&event, UCC_TL_UCP_IBCAST_DPU_DONE_AM_ID,
                            c.ep, c.id, NULL);
...
/* UCC_TL_UCP_IBCAST_HOST_ARRIVE_AM_ID */
static int
                  ...
                  size_t hdr_size, void *data, size_t data_len)
{
    ...
    ibcast_offload_coll_t *op = malloc(sizeof(ibcast_offload_coll_t));
    if (!op) {
        ucs_error("not enough memory");
        return UCS_ERR_NO_MEMORY;
    }
    ...
    ibcast_offload_args_t *args = &op->args;
    size_t unpacked_size = unpack_ibcast_offload_args(data, args);
    ...
    /* prepare subops */
    rc = create_ibcast_offload_subops(op);
    if (rc) {
        ucs_error("create_ibcast_offload_subops() failed");
        return rc;
    }
    ...
    ibcast_offload_subop_t *subop = NULL, *subop_item, *subop_tmp;
    ucs_list_for_each_safe(subop_item, subop_tmp, &op->s_pending, super) {
        ...
        /* get an event from event pool, allocate event buffer */
        size_t packed_size = get_ibcast_offload_dpu_rts_packed_size(
                                op->s_rkey_len);
        dpu_offload_event_info_t event_info = { .payload_size = packed_size, };
        dpu_offload_event_t *event;
              ...
              size_t data_len)
{
    ...
    ibcast_offload_dpu_rts_t *args = (ibcast_offload_dpu_rts_t *)data;
    int rc;

    /* find receiver from active_colls */
    ibcast_offload_coll_t *op = NULL, *op_item, *op_tmp;
    ...
    if (op == NULL) {
        ...
        ibcast_offload_dpu_rts_event_t *item =
            malloc(sizeof(ibcast_offload_dpu_rts_event_t));
        if (!item) {
            ucs_error("not enough memory");
            return UCS_ERR_NO_MEMORY;
        }
        ...
        ibcast_offload_dpu_rts_t *payload = malloc(data_len);
              ...
              size_t data_len)
{
    ...
    ibcast_offload_dpu_ack_t *args = (ibcast_offload_dpu_ack_t *)data;
    int rc;
...
static int register_ibcast_dpu_notifications(offloading_engine_t *engine)
{
    int rc;
...
static int progress_rts(execution_context_t *context)
{
    ...
    ibcast_offload_dpu_rts_event_t *rts = NULL, *rts_item, *rts_tmp;
    ibcast_offload_coll_t *op = NULL, *op_item, *op_tmp;
    ibcast_offload_subop_t *subop = NULL, *subop_item, *subop_tmp;
...
static int progress_receive(execution_context_t *context)
{
    ...
    ibcast_offload_rdma_read_t *recv = NULL, *recv_item, *recv_tmp;
    ibcast_offload_coll_t *op;
    ibcast_offload_subop_t *subop;
    ucs_list_for_each_safe(recv_item, recv_tmp, &posted_receives, super) {
        ...
        if (status == UCS_OK) {
            ...
            /* get an event from event pool, allocate event buffer */
            size_t packed_size = sizeof(ibcast_offload_dpu_ack_t);
            ...
            ibcast_offload_dpu_ack_t *args_buf =
                (ibcast_offload_dpu_ack_t *)sp_event->payload;
            args_buf->coll_type = op->args.coll_type;
            args_buf->tag       = op->args.tag;
            args_buf->group_id  = op->args.group_id;
            args_buf->rank      = op->args.rank;
    ...
    for (int i = 0; (i < max_receives - posted_count) &&
        ...
        ibcast_offload_rdma_read_t *item =
            ucs_list_extract_head(&pending_receives,
                                  ibcast_offload_rdma_read_t, super);
